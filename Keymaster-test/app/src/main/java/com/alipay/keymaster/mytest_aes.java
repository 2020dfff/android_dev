...
public class mytest_aes {
    ...
    static char sbox[] = {
            ...
            0x8c,0xa1,0x89,0x0d,0xbf,0xe6,0x42,0x68,0x41,0x99,0x2d,0x0f,0xb0,0x54,0xbb,0x16  /*f*/};//S盒表
    ...
    static char invsbox[] ={
            ...
            0x17,0x2b,0x04,0x7e,0xba,0x77,0xd6,0x26,0xe1,0x69,0x14,0x63,0x55,0x21,0x0c,0x7d  /*f*/};//逆S盒表
     ...
     public void KeyExpansion(char[] key) {
         ...
         while(i < 44){
             ...
             if(i%4==0){
                 ...
                 //与Rcon异或
                 temp[0] = (char) (temp[0] ^ Rcon[i/4-1]);
             }
             for (int k = 0; k < 4; k++){
                 RoundKey[i*4+k] = (char) (RoundKey[(i-4)*4+k] ^ temp[k]);
             }
             i++;
         }
     }
     ...
     * 有限域GF(28)上的乘法
     */
    public char FFmul(char a, char b){
        char bw[] = new char[4];
        char res=0;
        int i;
    ...
    public void MixColumns(char state[][]){
        ...
        for(c=0; c< 4; c++){
            ...
            for(r=0; r<4; r++){
                state[r][c] = (char) (FFmul((char) 0x02, t[r])           // 在GF(256)中，加法就是异或运算
     ...
     * 轮密钥加变换
     */
    public void AddRoundKey(char state[][],int round){
        for(int c = 0; c < 4; c++){
            for(int r = 0; r < 4; r++){
    ...
    public void InvMixColumns(char state[][]){
        ...
        for(c = 0; c < 4; c++){
            ...
            for(r = 0; r < 4; r++){
                state[r][c] = (char)(FFmul((char) 0x0e, temp[r])
                                    ^ FFmul((char) 0x0b, temp[(r+1)%4])
                                    ^ FFmul((char) 0x0d, temp[(r+2)%4])
                                    ^ FFmul((char) 0x09, temp[(r+3)%4]));
            }
        }
    }
